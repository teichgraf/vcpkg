...
{
  av_log(afq->avctx, AV_LOG_WARNING, "\nAFQ add to queue. Current count %d\n", afq->frame_count);
  for (int i = 0; i < afq->frame_count; i++) {
    av_log(afq->avctx, AV_LOG_WARNING, "AFQ frame[%d] : %d\n", i, afq->frames[i].pts);
  }
    ...
    if (f->pts != AV_NOPTS_VALUE) {
        ...
        if(afq->frame_count && new[-1].pts >= new->pts)
            av_log(afq->avctx, AV_LOG_WARNING, "Queue input is backward in time %d -- %d : %d\n", f->pts, new[-1].pts, new->pts);
        else if (afq->frame_count)
            av_log(afq->avctx, AV_LOG_WARNING, "Adding frame %d -- %d : %d\n", f->pts, new[-1].pts, new->pts);
    } else {
	...
	av_log(afq->avctx, AV_LOG_WARNING, "AFQ add complete. Current count %d\n", afq->frame_count);
    for (int i = 0; i < afq->frame_count; i++) {
      av_log(afq->avctx, AV_LOG_WARNING, "AFQ frame[%d] : %d\n", i, afq->frames[i].pts);
    }
...
void ff_af_queue_remove(AudioFrameQueue *afq, int nb_samples, int64_t *pts,
                        int64_t *duration)
{
	av_log(afq->avctx, AV_LOG_WARNING, "\nAFQ remove from queue. Current count %d\n", afq->frame_count);
    for (int i = 0; i < afq->frame_count; i++) {
      av_log(afq->avctx, AV_LOG_WARNING, "AFQ frame[%d] : %d\n", i, afq->frames[i].pts);
    }
	...
	av_log(afq->avctx, AV_LOG_WARNING, "\nAFQ remove complete. Current count %d\n", afq->frame_count);
    for (int i = 0; i < afq->frame_count; i++) {
      av_log(afq->avctx, AV_LOG_WARNING, "AFQ frame[%d] : %d\n", i, afq->frames[i].pts);
    }
}
